...
a, c, e는 조합할 여지가 있기 때문에 각각의 위치 중 하나를 골라

cFaBeD가 된다.

(S = 여러개의 c, e, a로 이루어진 랜덤 문자)

주어진 문자는 S + F + S + B + S + D + S 라고 하자.

그럼 결과는 a, c, e 모두 f보다 작기 때문에 가장 앞에 알파벳 순으로 정렬한다.

aceFBD

f와 d의 자리를 바꾸면 결과는 확실히 보일 것이다.

S + D + S + B + S + F + S

a, c는 d보다 앞이기 때문에 여전히 가장 앞에 알파벳 순으로 정렬되지만 e의 경우 d의 뒤쪽으로 밀린다. 여기에서 다음에 오는 b보다 순서에 밀리고 f보다 앞이기 때문에 acDBeF 가 된다.
...
def removeDuplicateLtters(s): # 가장 앞에 나올 알파벳만을 찾는 알고리즘이다. 이것을 반복하며 다음 순서 알파벳을 찾는다.
    for char in sorted(set(s)): #[a,b,c,d,e,f]
        suffix = s[s.index(char):] # char가 a라면 본래 입력 문자열에서 처음 나오는 a부터 끝까지를 suffix라고 한다. 
        if set(s) == set(suffix): # suffix의 set이 본래 문자열의 set과 같다면 해당 글자는 가장 앞에 오는 글자이다.
            return char + self.removeDuplicateLtters(suffix.replace(char, '')) # 해당 글자를 제거하고 남은 것에서 반복한다.
    return ''
